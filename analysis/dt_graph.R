...
tr_dep1 <- svyby(~forww+infww, ~year, subset(malesvy, male1824==1), svymean) 
tr_dep2 <- svyby(~rhwage, ~year, subset(malesvy, male1824==1 & x==1 & formal==1), svymean) %>% rename(forwage = rhwage) 
tr_dep3 <- svyby(~rhwage, ~year, subset(malesvy, male1824==1 & x==1 & formal==0), svymean) %>% rename(infwage = rhwage) 
tr_unemp <- svyby(~unemployed, ~year, subset(malesvy, male1824==1), svymean) 

# Descriptive statistics for control group (25-28 Aged Male)
cn_dep1 <- svyby(~forww+infww, ~year, subset(malesvy, male1824==0), svymean)
cn_dep2 <- svyby(~rhwage, ~year, subset(malesvy, male1824==0 & x==1 & formal==1), svymean) %>% rename(forwage = rhwage)
cn_dep3 <- svyby(~rhwage, ~year, subset(malesvy, male1824==0 & x==1 & formal==0), svymean) %>% rename(infwage = rhwage)
cn_unemp <- svyby(~unemployed, ~year, subset(malesvy, male1824==0), svymean) 

################################################################################
# Create table for descriptive statistics 
# Treatment Group
male1824 <- cbind(tr_dep1,tr_dep2,tr_dep3,tr_unemp) %>% select(c(var)) %>% round(digits = 3) %>% mutate(group="male1824")
# Control Group
male2528 <- cbind(cn_dep1,cn_dep2,cn_dep3,cn_unemp) %>% select(c(var)) %>% round(digits = 3) %>% mutate(group="male2528")
# Combine datasets
dt_graph <- rbind(male1824,male2528) 
# Write to file
